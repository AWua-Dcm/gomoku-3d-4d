...
>
> ⚠️ **`setTimeout` 的清理是本节最容易写错的地方。** 四个出口（同意 / 拒绝 / 被新动作取消 /
> 被重开取消）**全都必须** `clearTimeout`。漏掉任何一个，60 秒后那个定时器会**再结算一次**，
> 给双方播一条早就作废的 `undoResolved` —— 而且是在棋盘已经往前走了之后。
>
> 这个 bug 的可怕之处：**它只在"玩家没回应"的时候出现**。正常走一遍完整流程永远测不到。
> 所以 §9 的注入验证里专门有一条是「把 `clearTimeout` 删掉，测试必须变红」。
...
| 6 | **删掉 `cancelPending` 里的 `clearTimeout`** | 第 3 层：悔棋超时（**只跑一次流程抓不到，必须跑到超时之后**） |
